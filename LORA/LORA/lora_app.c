...
//		}
    ...
    if(distance_now < distance_threshold)                                       //当所测距离小于阈值
    {
      flag_carcame_now = 1;                                                     //将标志位置1
    }
    else
    {
      flag_carcame_now = 0;                                                     //将标志位置0
    }
    ...
    if(flag_carcame_now == 1)
    {
      GPIO_SetBits(GPIOA,GPIO_Pin_1);						                                //继电器输出高
      GPIO_ResetBits(GPIOA,GPIO_Pin_7);						                              //PA.7 输出低
    }
    else
    {
      GPIO_ResetBits(GPIOA,GPIO_Pin_1);						                              //继电器输出低
      GPIO_SetBits(GPIOA,GPIO_Pin_7);						                                //PA.7输出高
    }
